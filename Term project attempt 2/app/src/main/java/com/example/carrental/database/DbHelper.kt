...
class DbHelper private constructor(private var context: Context) : SQLiteOpenHelper(context, DATABASE_NAME,null, 2) {
    ...
    override fun onCreate(db: SQLiteDatabase) {
        //Will delete the table on startup (disabled) Keep disabled to maintain data across runs
//        delete()
        // Will recreate the table if it has been changed
        recreate()
    }
    ...
    private fun deleteAll(){
        ...
        val DROP_USER_TABLE = "DROP TABLE IF EXISTS $USER_TABLE_NAME"
        val DROP_CAR_TABLE = "DROP TABLE IF EXISTS $CAR_TABLE_NAME"
        val DROP_REVIEW_TABLE = "DROP TABLE IF EXISTS $REVIEW_TABLE_NAME"
        val DROP_RENTAL_TABLE = "DROP TABLE IF EXISTS $RENTAL_TABLE_NAME"
        db.execSQL(DROP_USER_TABLE)
        db.execSQL(DROP_CAR_TABLE)
        db.execSQL(DROP_REVIEW_TABLE)
        db.execSQL(DROP_RENTAL_TABLE)
    }
    ...
    private fun recreate(){
        ...
        val createUser = buildString {
            append("CREATE TABLE IF NOT EXISTS \"$USER_TABLE_NAME\" (\n")
        ...
        val createCar = buildString {
            append("CREATE TABLE IF NOT EXISTS \"$CAR_TABLE_NAME\" (\n")
            ...
            append("\t\"$CAR_COLUMN_ABAILABILITY\"\tTEXT,\n")
        ...
        val createReview = buildString {
            append("CREATE TABLE IF NOT EXISTS \"$REVIEW_TABLE_NAME\" (\n")
        ...
        val createRental = buildString {
            append("CREATE TABLE IF NOT EXISTS \"$RENTAL_TABLE_NAME\" (\n")
